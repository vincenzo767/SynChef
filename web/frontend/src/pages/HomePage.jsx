...
const getRecommendedRecipes = (countryCode, countryName) => {
  ...
  const recipeCountry =
    COUNTRY_CODE_TO_RECIPE_COUNTRY[countryCode?.toUpperCase()] || countryName;
  const exact = ALL_RECIPES.filter(
    (r) => r.country.toLowerCase() === recipeCountry?.toLowerCase()
  );
  ...
  const regional = region
    ? ALL_RECIPES.filter(
        (r) =>
          r.region === region &&
          r.country.toLowerCase() !== recipeCountry?.toLowerCase()
      )
    : [];
        ...
        <div className="hp-modal-img-wrap">
          ...
          <div className="hp-modal-hero-info">
            ...
            <div>
              ...
              <span className="hp-modal-country">
...
const CookingModal = ({ recipe, onClose }) => {
  ...
  const ringProgress =
    timer && step.timer ? (timer.remaining / step.timer) * 283 : 283;
  const activeTimerEntries = Object.entries(timers).filter(
          ...
          <div className="hp-ck-timers-bar">
            {activeTimerEntries.map(([k, t]) => {
              ...
              return (
                <div
                  key={k}
                  className={`hp-ck-chip ${t.running ? "running" : "paused"}`}
                >
                  <span>{s?.timerLabel || `Step ${+k + 1}`}</span>
                  <span className="hp-ck-chip-time">
              ...
              <div className="hp-ck-timer-area">
                {!timer?.started ? (
                  <button className="hp-ck-start-btn" onClick={startTimer}>
                    <FaPlay /> Start {step.timerLabel || "Timer"} (
                  ...
                  <div className="hp-ck-timer-display">
                    <div className="hp-ck-circle">
                      ...
                      <div
                        className={`hp-ck-circle-text ${timer.completed ? "done" : ""}`}
                      >
                        {timer.completed
                          ? <FaCheck />
                    ...
                    {!timer.completed && (
                      <button className="hp-ck-pause-btn" onClick={pauseResume}>
                        {timer.running
                          ? <Fragment><FaPause /> Pause</Fragment>
                          : <Fragment><FaPlay /> Resume</Fragment>}
                      </button>
                    )}
                    {timer.completed && (
                      <span className="hp-ck-done-label">
        ...
        <div className="hp-ck-nav">
          ...
          <div className="hp-ck-dots">
            {recipe.steps.map((_, i) => (
              <button
                key={i}
                className={`hp-ck-dot ${i === stepIndex ? "active" : ""} ${
                  i < stepIndex ? "done" : ""
                }`}
                onClick={() => goToStep(i)}
              />
          ...
          {!isLast ? (
            <button
              className="hp-ck-nav-btn primary"
              onClick={() => goToStep(stepIndex + 1)}
            >
              Next <FaArrowRight />
            </button>
...
const HomePage = () => {
  ...
  const { user, isAuthenticated, favoriteRecipeIds } = useSelector(
    (state) => state.auth
  );

  // Sync local favorites from Redux
  useEffect(() => {
    if (isAuthenticated) {
  ...
  const getStoredFavId = useCallback(
    (webId) => {
    ...
    <div className="home-page">
      ...
      <section className="hp-hero" aria-hidden="false">
        ...
        >
          ...
          <p className="hp-hero-sub">
            Explore authentic recipes from around the world, powered by
      ...
      <div className="home-hero-wrapper">
        ...
        <div className="home-hero-content">
          ...
          >
            Search authentic recipes from every corner of the world. Real
            ingredients, real culture, real cooking.
          </motion.p>
          <motion.div
        ...
        {/* Feature cards anchored to bottom of hero */}
        <motion.div
      ...
      <div className="synchef-gradient-bg">
        <div className="container">
          ...
          {/* ── Search + filters ──────────────────────────────── */}
          <section className="hp-body-section">
            <motion.div
              ...
              <div className="hp-search-row">
                ...
                <div className="hp-country-dropdown">
                  ...
                  >
                    ...
                    <FaChevronDown
                      className={countryDropdownOpen ? "rotated" : ""}
                    />
                  </button>
                  {countryDropdownOpen && (
          ...
          <section className="recipe-section">
            ...
            {visibleRecipes.length === 0 ? (
              <div className="empty-recipe-state">
                <FaSearch
                  style={{ fontSize: "2rem", opacity: 0.4, marginBottom: 10 }}
                />
                <p>
                  No recipes found — try a different search term or country.
                </p>
                <button
                  className="hp-clear-all-center"
